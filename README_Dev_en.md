...
Linux Electron builds keep Ubuntu 22.04 as their runtime baseline and use Clang 15. GCC 11 fails on the deprecated/visibility attribute combination in Electron 43 V8 headers. For local Linux source builds, install `clang-15` and run `CC=clang-15 CXX=clang++-15 node scripts/rebuild-electron.js all`. The compiler override applies only to Electron CI; Node-native jobs keep their existing compiler. Test/package jobs use pinned `ubuntu-24.04` runners. Validate toolchain changes with an actual Linux Electron build, not only a macOS build.
