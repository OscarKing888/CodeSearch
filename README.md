...
- **删除 Available 索引**：管理页中的 **Delete** 会在显示完整数据库路径并二次确认后，永久删除对应数据库及 SQLite WAL/SHM 数据；正在使用、被其他记录引用或被其他 IDE 锁定的索引会拒绝删除
- **兼容旧索引**：原 VS Code/Cursor `globalStorage` 索引不会被自动搬移或删除，会以 Legacy 来源继续打开；`code-search.autocreate` 仍优先并控制其配置路径
...
`search_class_hierarchy` 使用大小写敏感类名匹配；限定名唯一定位，同名短类名会返回候选声明位置。结果采用扁平 DAG 以保留 C++ 多继承。`maxNodes` 支持 1–5000 或 `"all"`；省略时读取共享用户设置 `codeSearch.mcpClassHierarchyDefaultMaxNodes`（默认 20，0 表示全部）。工具优先复用当前 Class Hierarchy 缓存，缺失或过期部分只在内存中解析索引快照，不写数据库。

### 索引发现与只读保证

- 无参数启动时容错发现 VS Code 与 Cursor `globalStorage` 中的 registry；损坏的自动来源会出现在 `list_indexes.warnings`，不会阻止 Server 启动
- 默认只暴露 MCP client roots 完全包含的索引；兼容 Cursor 返回的 Windows 绝对路径和标准 `file://` URI。客户端声明支持 roots 后，空值、无效值或请求失败会清空该会话作用域，不会回退到可能过宽的 cwd；只有不支持 roots 的旧客户端才使用 `--workspace-root` / cwd。父目录索引、混合无关根的索引会安全拒绝，跨工作区必须显式使用 `--all-indexes`
...
项目指导只保留在 `.agents`，当前 Codex、VS Code/Copilot 和 Cursor 共用这一个 Skill。普通安装**不会**在项目中创建 `.codex`、`.github`、`.cursor` 或 `.claude` 指导文件，扩展激活时也不会静默写入。检测到用户修改、无效 marker 或自定义 MCP 配置时会保留原内容并警告。

搜索面板状态栏中间显示 MCP 运行状态：没有当前工作区服务时为灰色 `Waiting`，服务心跳可用时为绿色 `Ready`，收到工具请求时以黄色显示经过截断和脱敏的可读动作摘要，例如 `正在搜索 “xxx”`、`正在读取 File.ts:12–20` 或 `正在获取索引`。VS Code、Cursor 和 Codex 启动的多个 MCP 进程会按工作区聚合显示；聚合仅用于状态提示，每个 IDE 实例仍使用独立的 stdio 会话、索引作用域和工具响应。
...
> 旧版由扩展管理且未被修改的项目 `.cursor/skills`、`.cursor/rules`、`.claude/skills`、项目 `.codex` 和 `.github/instructions` 会安全迁移/清理；无法确认所有权或已被修改的文件会保留并警告。完整细节见 [README_Dev.md — MCP (AI Agent)](README_Dev.md#mcp-ai-agent)。
...
| **AI Agent** | 只读 stdio MCP | ✅ 索引发现、搜索、快照读取、类继承、头源配对 |
| | Skill / 搜索优先规则分发 | ✅ Codex、VS Code/Copilot、Cursor 共用 `.agents` 唯一 Skill |
| | MCP 服务状态 | ✅ Waiting / Ready / 安全请求摘要，跨进程工作区聚合 |
| **搜索** | 单词 / 多词 / 短语 | ✅ |
| | 通配符 `*`（单词级） | ✅ |
...
| Refresh Index | 命令面板 |
| Manage Indexes | 工具栏 ⚙ |
| Install Agent Skill | 工具栏文档勾选图标 / 命令面板（只向当前项目写入 `.agents`） |
