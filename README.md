...
工具栏「Install project Agent Skill / Rule / MCP」或命令 **Ace Code Search: Install Project Agent Skill, Rule, and MCP Config** 会写入：

- 项目 Skill（Codex）：`.agents/skills/ace-code-search-mcp`
- 项目 Skill（Cursor）：`.cursor/skills/ace-code-search-mcp`
- 项目 Rule（Cursor）：`.cursor/rules/ace-code-search-first.mdc`
- 项目 Instruction（VS Code Copilot）：`.github/instructions/ace-code-search.instructions.md`
- **Codex MCP**：`~/.codex/config.toml` 的 `[mcp_servers.ace-code-search]`（以及项目 `.codex/config.toml`）
- **Cursor MCP**：`~/.cursor/mcp.json` 的 `ace-code-search`

> Skill 只提供用法说明；**没有 MCP Server 注册时，会话里不会出现** `list_indexes` / `search_code` 等工具。安装后请重启 Codex（或执行 `/mcp`）再试。

这些 Skill/Rule 文件建议提交进仓库。扩展**不会**在激活时静默写入。
...
> Skill/Rule 分发与 MCP Server 注册以前是两件事；现在工具栏安装会**同时**写入 Codex/Cursor 的 MCP 客户端配置。完整示例见 [README_Dev.md — MCP (AI Agent)](README_Dev.md#mcp-ai-agent)。
...
| | Skill / 搜索优先规则分发 | ✅ 写入当前项目（Codex `.agents` + Cursor/VS Code） |
| **搜索** | 单词 / 多词 / 短语 | ✅ |
| | 通配符 `*`（单词级） | ✅ |
