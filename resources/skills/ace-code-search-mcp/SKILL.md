...
The Codex/Cursor launcher starts with the client `node` command, but before loading MCP it re-execs to a compatible Node.js 20/22/24 runtime when the PATH Node ABI does not match the packaged `native-node/` binaries (for example Node 23 on Homebrew). It prefers `ACE_CODE_SEARCH_NODE`, then Cursor's helper Node when installed, then versioned PATH or Homebrew candidates. The packaged MCP entry repeats this check so a launcher installed by an earlier extension can recover after upgrading. The VSIX guarantees Node.js 20, 22, and 24 ABIs. VS Code's dynamic provider uses the editor runtime instead and does not require a separate PATH Node.
